...
public class ChaosGenerator {
	...
	public static ArrayList<ChaosNumber> generateChaosNumbers(double xMin, double xMax, double yMin, double yMax, int numberOfSamples){
		...
		if ((Math.sqrt(numberOfSamples) % 1) != 0) {
			remainder = (int)Math.ceil((numberOfSamples - Math.pow(Math.floor(Math.sqrt(numberOfSamples)),2)) / ySamples);
		}
	...
	public static Vector3d getLyapunovValuesNoNormalization(ArrayList<SpaceObjectFast> spaceObjects, SpaceObjectFast probe, int numOfSamples, double deltaD, double timeStep, int numOfIterations) {
		...
		for(int x = 0; x<numOfSamples; x++) {
			...
			Vector3d deltaPlacement = new Vector3d((double)(deltaD*Math.cos(2*Math.PI*((double)x/(numOfSamples)))),0,(double)(deltaD*Math.sin(2*Math.PI*((double)x/(numOfSamples)))));
			for(SpaceObjectFast s : spaceObjects) {
				if (!s.getName().equals(probe.getName())) {
		...
		return new Vector3d(Math.log(maxDelta/deltaD)/(timeStep*numOfIterations), Math.log(minDelta/deltaD)/(timeStep*numOfIterations), 0);
	}
	
	public static Vector3d getLyapunovValuesWithNormalization(ArrayList<SpaceObjectFast> spaceObjects, SpaceObjectFast probe, int numOfSamples, double deltaD, double timeStep, int numOfIterations) {
		ArrayList<ArrayList<SpaceObjectFast>> spaceObjectsTest = new ArrayList<ArrayList<SpaceObjectFast>>();
		ArrayList<SpaceObjectFast> spaceObjectsMain = new ArrayList<SpaceObjectFast>();
		for(SpaceObjectFast s : spaceObjects) {
			if (!s.getName().equals(probe.getName())) {
	...
	public static Vector3d getStickyStability(ArrayList<SpaceObjectFast> spaceObjects, SpaceObjectFast probe, int numOfSamples, double deltaD, double timeStep, int numOfIterations) {
		ArrayList<SpaceObjectFast> spaceObjectsMain = new ArrayList<SpaceObjectFast>();
		for(SpaceObjectFast s : spaceObjects) {
			if (!s.getName().equals(probe.getName())) {
				SpaceObjectFast clone = s.clone();
		...
		for(SpaceObjectFast s : spaceObjectsMain) {
			if (!s.getName().equals(probe.getName())) {
				initialRelativeDistances.put(index, spaceObjectsMain.get(indexOfProbe).getToScalePosition().distance(s.getToScalePosition()));
			}
			index++;
		}
		...
		for(int n = 0; n < numOfIterations; n++) {
			...
			for(SpaceObjectFast s : spaceObjectsMain) {
				if (!s.getName().equals(probe.getName())) {
					...
					netDifferences += Math.abs(initialDistance - spaceObjectsMain.get(indexOfProbe).getToScalePosition().distance(s.getToScalePosition()))/initialDistance;
				}
				index++;
			}
		}
	...
	public static Vector3d getChaosValues(ArrayList<SpaceObjectFast> spaceObjects, SpaceObjectFast probe, int numOfSamples, double deltaD, double timeStep, int numOfIterations) {
		ArrayList<ArrayList<SpaceObjectFast>> spaceObjectsTest = new ArrayList<ArrayList<SpaceObjectFast>>();
		ArrayList<SpaceObjectFast> spaceObjectsMain = new ArrayList<SpaceObjectFast>();
		for(SpaceObjectFast s : spaceObjects) {
			if (!s.getName().equals(probe.getName())) {
		...
		return new Vector3d(maxDelta/(timeStep*numOfIterations), minDelta/(timeStep*numOfIterations), Math.log(netDifferences)/(numOfIterations*timeStep));
	}
	
	public static String getChaosSetupAsString(ArrayList<SpaceObject> spaceObjects, SpaceObject probe, int[] parameters, double[] ranges, Class<? extends Simulation> simulationClass) {
